...
GLuint skyboxTexture;  // Global variable to hold the texture ID
...
int main()
{
    ...
    skyboxTexture = loadCubeMap({
        "Yokohama/posx.jpg", "Yokohama/negx.jpg",
        "Yokohama/posy.jpg", "Yokohama/negy.jpg",
        "Yokohama/posz.jpg", "Yokohama/negz.jpg"
    });
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        { // For 8 rows
            ...
            { // For 8 columns
                if ((i + j) % 2 == 0)
                {                                                  // Check if i+j is odd or even for color purposes
        ...
        // CONTROLS TO MOVE CYLINDER
        glm::mat4 view_cylinder = view;                                                // Create mat4 view_cylinder using generic view identity
        view_cylinder = glm::translate(view_cylinder, glm::vec3(-2.4f, -2.9f, -5.0f)); // Translate cylinder back, to the right, and down
        ...
        // Update the camera position uniform for reflective calculations
        glUniform3f(glGetUniformLocation(sphereShader.Program, "cameraPos"),
                    camera.Position.x, camera.Position.y, camera.Position.z);

        // The below uniforms are for lighting calculations; keep them if the shader still uses them for lighting effects
        ...
        modelLoc = glGetUniformLocation(sphereShader.Program, "model");
        viewLoc = glGetUniformLocation(sphereShader.Program, "view");
        projLoc = glGetUniformLocation(sphereShader.Program, "projection");

        glm::mat4 view_sphere = view;                                            // Start with the generic view matrix
        view_sphere = glm::translate(view_sphere, glm::vec3(2.0f, 0.0f, -5.0f)); // Move the sphere into position
        view_sphere = glm::scale(view_sphere, glm::vec3(0.5f, 0.5f, 0.5f));      // Scale down the sphere

        glUniformMatrix4fv(viewLoc, 1, GL_FALSE, glm::value_ptr(view_sphere)); // Pass view_sphere to the shader
        glUniformMatrix4fv(projLoc, 1, GL_FALSE, glm::value_ptr(projection));  // Pass projection matrix to the shader
        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));      // Pass model matrix to the shader

        sphereModel.Draw(sphereShader); // Draw sphere obj model

        glBindVertexArray(0);    // Unbind VAO to clean up
...
void do_movement()
{
    ...
    { // If either shift keys are pressed
        if (keys[GLFW_KEY_UP])
        {                                               // If up arrow
